...
class _ClassementPageState extends State<ClassementPage> {
  ...
  TextStyle _neonPinkStyle({double fontSize = 10, bool isGray = false}) {
    if (isGray)
      return TextStyle(
          color: Colors.white24, fontSize: fontSize, letterSpacing: 0.5);
  ...
  Future<void> _partagerTop10() async {
    try {
        ...
        child: Column(
          ...
          children: [
            ...
            ...top10.asMap().entries.map((entry) {
              ...
              bool notConfirmed =
                  p.artist.toLowerCase().contains("confirmer") ||
                      p.artist.toLowerCase().contains("inconnu");
              ...
              return Container(
                ...
                child: Row(
                  children: [
                    ...
                    CountryFlag.fromCountryCode(p.countryCode,
                        height: 15, width: 25),
                    const SizedBox(width: 15),
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(p.country,
                              style: const TextStyle(
                                  ...
                                  fontWeight: FontWeight.bold,
                                  fontSize: 13)),
                          Text(p.artist.toUpperCase(),
                              style: _neonPinkStyle(
                                  fontSize: 8, isGray: notConfirmed)),
                          Row(
                            children: [
                              ...
                              Expanded(
                                  child: Text(p.song,
                                      style: _neonCyanStyle(fontSize: 9),
                    ...
                    Text("${p.points} pts",
                        style: TextStyle(
                            color: index < 3 ? rankColor : Colors.cyanAccent,
                            fontWeight: FontWeight.bold)),
                  ],
                ),
              );
      ...
      final Uint8List? imageBytes = await _screenshotController
          .captureFromWidget(Material(child: widgetAPartager),
              delay: const Duration(milliseconds: 50));

      if (imageBytes != null) {
        final XFile xFile = XFile.fromData(imageBytes,
            mimeType: 'image/png', name: 'mon_top10.png');
        await Share.shareXFiles([xFile],
            text: 'Voici mon Top 10 Eurovision 2026 ! #Eurovision #EuroVote');
      }
    } catch (e) {
  ...
  Future<void> _fetchFromServer(String route) async {
    ...
    try {
      ...
      print('CODE REPOSNE API: ${response.statusCode}'); // AJOUTE ÇA
      print('CORPS DE L ERREUR: ${response.body}'); // ET ÇA
  ...
  void _decoderJson(String rawJson) {
    ...
    Map<String, int> pointsExistants = {
      for (var p in participants) p.countryCode: p.points
    };
  ...
  void _chargerSecours() {
    setState(() {
      participants = [
        Participant(
            country: "Erreur Serveur",
            countryCode: "FR",
            artist: "À confirmer",
            song: "Pas de connexion")
      ];
      isLoading = false;
    });
  }
  ...
  void _sauvegarderMonTop() {
    ...
    List<Map<String, dynamic>> toSave = participants
        .map((p) => {
              'country': p.country,
    ...
    String nomSauvegarde =
        "Top_${DateTime.now().day}_${DateTime.now().hour}h${DateTime.now().minute}";
    box.put(nomSauvegarde, toSave);

    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        backgroundColor: const Color(0xFFFE0191),
        content: Text("Top sauvegardé : $nomSauvegarde")));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("EUROVISION 2026",
        ...
        actions: [
          IconButton(
              ...
              onPressed: _partagerTop10),
          IconButton(
              icon: const Icon(Icons.leaderboard, color: Colors.amberAccent),
              onPressed: _trierAvecAnimation),
          IconButton(
              icon: const Icon(Icons.cloud_sync, color: Colors.cyanAccent),
              onPressed: () => _fetchFromServer('refresh')),
          IconButton(
              icon: const Icon(Icons.save, color: Color(0xFFFE0191)),
              onPressed: _sauvegarderMonTop),
          const SizedBox(width: 10),
        ],
      ),
      body: isLoading
          ? const Center(
              child: CircularProgressIndicator(color: Color(0xFFFE0191)))
                        ...
                        child: Row(
                          children: [
                            ...
                            ClipRRect(
                              ...
                              child: CountryFlag.fromCountryCode(p.countryCode,
                                  height: 20, width: 30),
                            ),
                          ],
                        ),
                      ...
                      trailing: Container(
                        ...
                        child: DropdownButton<int>(
                          ...
                          icon: Icon(Icons.arrow_drop_down,
                              color:
                                  isTop3 ? rankColor : const Color(0xFFFE0191),
                              size: 20),
                          items: pointsEurovision
